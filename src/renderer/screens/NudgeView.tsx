...
export default function NudgeView() {
  ...
  useEffect(() => {
    window.electronAPI.getSettings().then(s => {
      const settings = s as Partial<AppSettings>;
      if (settings.accentColor) setAccent(ACCENT_MAP[settings.accentColor] ?? '#2dd4bf');
    });
  ...
  return (
    <div style={{
      ...
      {nudge ? (
        ...
        <div style={{ fontSize: 11, color: '#525252', padding: 20 }}>loading...</div>
      )}
    </div>
  );
}
